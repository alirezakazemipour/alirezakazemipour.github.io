...
On the other hand to compute $P_{1000}(2, 4)$, since the chain is periodic the and the remainder of $1000$ when when divided by $3$ is one the same as number $4$, hence using Python software we raise the matrix to the power of $4$ instead, and $P_{1000}(2, 4) \approx P_4(2, 4) \approx  0.4167$.
$$
\begin{equation*}
    P_4 = \begin{pmatrix}
  0 & 0.3333 & 0.6665 & 0 & 0\\
...
---
- Let $X_1, X_2,\dots$ be the successive values from independent rolls of astandard six-sided die. Let $S_n= X_1 + \dots + X_n$. Let
$$
...
Now for each fixed $j$, $(i + j \bmod{8})$ is a permutation of $\{0, 1, 2, 3, 4, 5, 6, 7\}$ excluding the number $j$. Hence, by denoting $S \coloneqq \sum_{i = 1}^7 g(i)$, we have:
